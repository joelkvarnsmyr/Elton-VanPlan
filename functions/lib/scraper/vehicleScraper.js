    ...
    SOURCES: [
        {
            ...
            name: 'car.info',
            enabled: true,
            buildUrl: (regNo) => `https://www.car.info/sv-se/license-plate/S/${regNo}`
        },
        {
            id: 'biluppgifter',
            name: 'biluppgifter.se',
            enabled: true,
            buildUrl: (regNo) => `https://biluppgifter.se/fordon/${regNo.toLowerCase()}/`
        },
        {
            id: 'transportstyrelsen',
            name: 'Transportstyrelsen',
            enabled: true,
            buildUrl: (regNo) => `https://fordon-fu-regnr.transportstyrelsen.se/UppgifterAnnatFordon?LicensePlate=${regNo}`
        }
...
async function scrapeCarInfo(regNo) {
    ...
    if (h1Text) {
        ...
        if (h1Text.includes('Manuell'))
            data.gearbox = 'Manuell';
        else if (h1Text.includes('Automat'))
            data.gearbox = 'Automat';
    }
    // Extract specs
    ...
    if (regDate)
        data.firstRegistered = parseSwedishDate(regDate);
    const lastInsp = getSpec('Senaste besiktning');
    if (lastInsp)
        data.lastInspection = parseSwedishDate(lastInsp);
    const nextInsp = getSpec('Nästa besiktning');
    if (nextInsp)
        data.nextInspection = parseSwedishDate(nextInsp);
    ...
    try {
        ...
        if (scriptContent && scriptContent[1]) {
            ...
            if (Array.isArray(mileageData) && mileageData.length > 0) {
                data.mileageHistory = mileageData.map((entry) => ({
...
async function scrapeBiluppgifter(regNo) {
    ...
    if (firstReg)
        data.firstRegistered = parseSwedishDate(firstReg);
    data.ownerCount = parseSwedishNumber(getSpec('Antal ägare')) || undefined;
    const lastOwner = getSpec('Senaste ägarbyte');
    if (lastOwner)
        data.lastOwnerChange = parseSwedishDate(lastOwner);
...
async function scrapeTransportstyrelsen(regNo) {
    ...
    const getAllFromSection = (sectionId) => {
        const pairs = {};
 ...
 */
async function aiMergeVehicleData(regNo, sourceResults, apiKey) {
...
RETURNERA JSON enligt detta schema (inga kommentarer, bara valid JSON):
{
  ...
  "history": {
    ...
    try {
        ...
        return adaptToVehicleData(successfulSources[0].data, successfulSources.map(s => s.sourceName));
    }
}
/**
 * Direkt adapter från RawVehicleData till VehicleData (utan AI)
 */
function adaptToVehicleData(raw, sources) {
    return {
        regNo: raw.regNo || '',
        make: raw.make || 'Okänt',
...
async function setCachedVehicleData(regNo, vehicleData, sources) {
    const db = admin.firestore();
    const docRef = db.collection('vehicleDataCache').doc(regNo);
...
exports.scrapeVehicleData = (0, https_1.onCall)({
    ...
    try {
        ...
        if (apiKey && successfulSources.length > 1) {
            ...
            vehicleData = adaptToVehicleData(successfulSources[0].data, successfulSources.map(s => s.sourceName));
        }
