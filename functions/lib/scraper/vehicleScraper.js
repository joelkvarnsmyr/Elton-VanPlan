...
async function scrapeCarInfo(regNo) {
    ...
    try {
        ...
        if (owners)
            vehicleData.history.owners = parseSwedishNumber(owners);
        const mileage = getSpec('Mätarställning');
        if (mileage)
            vehicleData.inspection.mileage = mileage;
        const bodyType = getSpec('Kaross');
        if (bodyType)
            vehicleData.bodyType = bodyType;
        ...
        if (power)
            vehicleData.engine.power = power;
        // const horsePower = getSpec('Hästkrafter');
        // Already have power in kW, this is redundant
        const engineVolume = getSpec('Motorvolym');
        if (engineVolume)
            vehicleData.engine.volume = engineVolume;
        const fuel = getSpec('Bränsle');
        if (fuel)
            vehicleData.engine.fuel = fuel;
        // const tankVolume = getSpec('Tankvolym');
        // Can add to engine if needed
        // Transmission & Drive
        const drive = getSpec('Drivlina');
        if (drive)
            vehicleData.wheels.drive = drive;
        const gearbox = getSpec('Växellåda');
        if (gearbox)
            vehicleData.gearbox = gearbox;
        // Dimensions & Weights
        const length = getSpec('Längd');
        if (length)
            vehicleData.dimensions.length = parseSwedishNumber(length);
        const width = getSpec('Bredd');
        if (width)
            vehicleData.dimensions.width = parseSwedishNumber(width);
        const height = getSpec('Höjd');
        if (height)
            vehicleData.dimensions.height = height;
        const curbWeight = getSpec('Tjänstevikt');
        if (curbWeight)
            vehicleData.weights.curb = parseSwedishNumber(curbWeight);
        const totalWeight = getSpec('Totalvikt');
        if (totalWeight)
            vehicleData.weights.total = parseSwedishNumber(totalWeight);
        const trailerWeight = getSpec('Släpvagnsvikt');
        if (trailerWeight)
            vehicleData.weights.trailer = parseSwedishNumber(trailerWeight);
        const trailerBWeight = getSpec('Släpvagnsvikt obromsad');
        if (trailerBWeight)
            vehicleData.weights.trailerB = parseSwedishNumber(trailerBWeight);
        // Registration info
        const regDate = getSpec('Första registrering');
        if (regDate)
            vehicleData.regDate = parseSwedishDate(regDate);
        // Inspection
        const lastInspection = getSpec('Senaste besiktning');
        if (lastInspection)
            vehicleData.inspection.last = parseSwedishDate(lastInspection);
        const nextInspection = getSpec('Nästa besiktning');
        if (nextInspection)
            vehicleData.inspection.next = parseSwedishDate(nextInspection);
        // Tires
        const tiresFront = getSpec('Däck fram');
        if (tiresFront)
            vehicleData.wheels.tiresFront = tiresFront;
        const tiresRear = getSpec('Däck bak');
        if (tiresRear)
            vehicleData.wheels.tiresRear = tiresRear;
        const boltPattern = getSpec('Bultmönster');
        if (boltPattern)
            vehicleData.wheels.boltPattern = boltPattern;
        ...
        try {
            ...
            if (scriptContent && scriptContent[1]) {
                ...
                if (Array.isArray(mileageData) && mileageData.length > 0) {
                    vehicleData.history.mileageHistory = mileageData.map((entry) => ({
...
async function scrapeBiluppgifter(regNo) {
    ...
    try {
        ...
        if (firstReg)
            vehicleData.regDate = parseSwedishDate(firstReg);
        const owners = getSpec('Antal ägare');
        if (owners)
            vehicleData.history.owners = parseSwedishNumber(owners);
        const lastOwnerChange = getSpec('Senaste ägarbyte');
        if (lastOwnerChange)
            vehicleData.history.lastOwnerChange = parseSwedishDate(lastOwnerChange);
        ...
        if (volume)
            vehicleData.engine.volume = volume;
        ...
        if (wheelbase)
            vehicleData.dimensions.wheelbase = parseSwedishNumber(wheelbase);
        const curbWeight = getSpec('Tjänstevikt');
        if (curbWeight)
            vehicleData.weights.curb = parseSwedishNumber(curbWeight);
        const totalWeight = getSpec('Totalvikt');
        ...
        if (loadWeight)
            vehicleData.weights.load = parseSwedishNumber(loadWeight);
        const trailerWeight = getSpec('Släpvagnsvikt');
        if (trailerWeight)
            vehicleData.weights.trailer = parseSwedishNumber(trailerWeight);
        // Tires
 ...
 */
async function getCachedVehicleData(regNo) {
 ...
 */
async function setCachedVehicleData(regNo, vehicleData, source) {
    const db = admin.firestore();
    const docRef = db.collection('vehicleDataCache').doc(regNo);
    const now = admin.firestore.Timestamp.now();
    const expiresAt = admin.firestore.Timestamp.fromMillis(now.toMillis() + (CONFIG.CACHE_TTL_SECONDS * 1000));
    const cacheData = {
        vehicleData,
        source,
        scrapedAt: now,
 ...
 */
function fillDefaultValues(partial) {
    return {
        regNo: partial.regNo || '',
...
//# sourceMappingURL=vehicleScraper.js.map