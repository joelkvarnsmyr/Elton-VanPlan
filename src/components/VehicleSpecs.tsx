...
export const VehicleSpecs: React.FC<VehicleSpecsProps> = ({
    ...
    return (
        <div className="space-y-6 pb-20 animate-fade-in">
            ...
            {activeTab === 'overview' && (
                <>
                    {/* Weight Watcher (Only for renovation/conversion) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        ...
                        <Card title="Historik & Status" icon={FileText} warning={true} onEdit={startEditing}>
                            <Row label="Registreringsnummer" value={vehicleData.regNo} highlight />
                            <Row label="Ramnummer (VIN)" value={vehicleData.vin || '-'} editKey="vin" />
                                            ...
                                            {vehicleData.expertAnalysis.commonFaults.map((fault, i) => (
                                                <li key={i} className="bg-white dark:bg-nordic-charcoal p-3 rounded-xl border border-amber-100 dark:border-amber-900/30 shadow-sm">
                                                    <div className="flex justify-between items-start mb-1">
                                                        ...
                                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${fault.urgency === 'High' ? 'bg-rose-100 text-rose-700' :
            ...
            {/* Disclaimer at the very bottom of the dashboard view */}
            <Disclaimer />
        </div>
    );
};
