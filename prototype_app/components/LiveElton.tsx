...
import { BASE_SYSTEM_PROMPT, LIVE_PERSONAS } from '../prompts';
...
export const LiveElton: React.FC<LiveEltonProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    const persona = LIVE_PERSONAS.find(p => p.id === currentPersonaId) || LIVE_PERSONAS[0];
  ...
  const startSession = async (persona: typeof LIVE_PERSONAS[0], diagnostic: boolean) => {
    try {
