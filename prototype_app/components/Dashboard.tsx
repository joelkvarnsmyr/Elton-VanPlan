...
import { Coins, CheckCircle2, Wallet, Calendar, MapPin, Flag, ArrowRight, ArrowUp, ArrowDown, History, Wrench } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ project, onPhaseClick }) => {
  ...
  const [sectionOrder, setSectionOrder] = useState<string[]>(() => {
      ...
      return saved ? JSON.parse(saved) : ['timeline', 'stats', 'budget', 'phases', 'resources'];
  });
  ...
  // Combined Timeline: History + Completed Tasks
  const timelineData = useMemo(() => {
    // 1. Static History Events
    const historyEvents = [
        {
            date: vehicleData.prodYear ? vehicleData.prodYear.toString() : '1976',
            title: 'Tillverkad',
            description: `Hannover, Tyskland`,
            type: 'history',
            icon: Calendar
        },
        {
            date: vehicleData.regDate || '1978',
            title: 'I Trafik',
        ...
        },
        {
            ...
            title: 'Projektstart',
            description: 'Elton flyttar in',
            type: 'milestone',
            icon: Flag
        }
    ];

    // 2. Completed Tasks (Dynamic)
    // We don't have a 'completedAt' date in the Task interface yet, so we use 'Klart' or a placeholder.
    // In a real app, we would sort by actual completion date.
    const completedTasks = tasks
        .filter(t => t.status === TaskStatus.DONE)
        .map(t => ({
            date: 'Nyligen', // Placeholder for now
            title: t.title,
            description: 'Avklarad uppgift',
            type: 'task',
            icon: CheckCircle2
        }));

    // Merge and simple sort (putting history first)
              ...
              return (
                <div key="timeline" className="bg-white dark:bg-nordic-dark-surface p-8 rounded-3xl shadow-sm border border-nordic-ice dark:border-nordic-dark-bg overflow-hidden">
                    ...
                    <div className="relative">
                        ...
                        <div className="flex overflow-x-auto pb-6 space-x-8 px-4 scrollbar-thin scrollbar-thumb-nordic-blue/50 scrollbar-track-transparent snap-x">
                            {timelineData.map((event, idx) => (
                                <div key={idx} className="relative flex flex-col items-center min-w-[140px] snap-center group">
                                    ...
                                    <div className="text-center transition-opacity duration-300">
                                        ...
                                        <h4 className="font-bold text-nordic-charcoal dark:text-nordic-ice text-sm leading-tight mb-1 line-clamp-2">{event.title}</h4>
                                        <p className="text-xs text-slate-500 dark:text-nordic-dark-muted max-w-[120px] mx-auto leading-relaxed truncate">{event.description}</p>
                                    </div>
                                </div>
                            ))}
                            <div className="min-w-[50px]"></div> {/* Spacer */}
                        </div>
                    </div>
                </div>
              );
