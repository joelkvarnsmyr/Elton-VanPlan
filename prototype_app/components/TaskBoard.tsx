...
import { Task, TaskStatus, Priority, ShoppingItem, VehicleData, Project } from '../types';
import { ChevronDown, ChevronRight, Check, AlertCircle, MessageSquare, Paperclip, Link as LinkIcon, ListChecks, CalendarClock, Lightbulb, Sparkles } from 'lucide-react';
...
export const TaskBoard: React.FC<TaskBoardProps> = ({ project, tasks, shoppingItems, vehicleData, onUpdateTask, initialFilter = 'ALL' }) => {
  const [expandedPhase, setExpandedPhase] = useState<string | null>(null);
  const [activeFilter, setActiveFilter] = useState<string | 'ALL' | 'IDEAS'>(initialFilter);
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);

  // Sync with prop changes (e.g. from Dashboard click)
  useEffect(() => {
    if (initialFilter !== 'ALL' && initialFilter !== 'IDEAS') {
  ...
  // Show filtered phases OR the Ideas board
  const showIdeas = activeFilter === 'IDEAS';
  const filteredPhases = activeFilter === 'ALL' || activeFilter === 'IDEAS' ? phases : [activeFilter];
  ...
  const getStatusColor = (status: TaskStatus) => {
    switch (status) {
      ...
      case TaskStatus.IDEA: return 'bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/40 dark:text-amber-200 dark:border-amber-900';
  ...
  // Render the Ideas Board
  if (showIdeas) {
      const ideas = tasks.filter(t => t.status === TaskStatus.IDEA);
      return (
          <div className="space-y-6 pb-20">
              <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
                <button onClick={() => setActiveFilter('ALL')} className="px-4 py-2 rounded-full text-sm font-medium bg-white text-slate-500 hover:bg-slate-50 dark:bg-nordic-dark-surface dark:text-nordic-dark-muted">Alla Faser</button>
                <button onClick={() => setActiveFilter('IDEAS')} className="px-4 py-2 rounded-full text-sm font-medium bg-amber-100 text-amber-800 shadow-md">Idébanken 💡</button>
              </div>

              <div className="bg-amber-50/50 dark:bg-amber-900/10 p-8 rounded-3xl border border-amber-100 dark:border-amber-900/30 text-center mb-6">
                  <Lightbulb size={48} className="mx-auto text-amber-400 mb-4" />
                  <h3 className="font-serif font-bold text-2xl text-amber-900 dark:text-amber-100">Idébanken</h3>
                  <p className="text-amber-700 dark:text-amber-300">Här samlar vi drömmar och lösa tankar som inte är redo för att bli "Måsten" än.</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {ideas.length === 0 && <div className="col-span-full text-center text-slate-400 italic">Inga idéer än. Be Elton lägga till en!</div>}
                  {ideas.map(task => (
                      <div key={task.id} onClick={() => setSelectedTask(task)} className="bg-white dark:bg-nordic-dark-bg p-5 rounded-2xl border border-amber-200 dark:border-amber-900 shadow-sm cursor-pointer hover:shadow-md transition-all relative group">
                          <div className="absolute top-4 right-4 text-amber-300 group-hover:text-amber-500 transition-colors"><Sparkles size={16} /></div>
                          <h4 className="font-bold text-lg text-nordic-charcoal dark:text-nordic-ice mb-2">{task.title}</h4>
                          <p className="text-sm text-slate-500 dark:text-nordic-dark-muted line-clamp-3">{task.description}</p>
                          <div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-800 flex justify-between items-center">
                              <span className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-md">Idé</span>
                              <button 
                                onClick={(e) => { e.stopPropagation(); onUpdateTask({...task, status: TaskStatus.TODO}); }}
                                className="text-xs font-bold text-teal-600 hover:bg-teal-50 px-2 py-1 rounded transition-colors"
                              >
                                Gör till Uppgift →
                              </button>
                          </div>
              ...
              {selectedTask && (
                <TaskDetailModal 
                    ...
                    onUpdate={(updated) => { onUpdateTask(updated); setSelectedTask(updated); }}
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
        ...
        <button 
           ...
           className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all bg-white text-slate-500 hover:bg-slate-50 dark:bg-nordic-dark-surface border border-slate-100 hover:text-amber-600`}
        >
          Idébanken 💡
        </button>
        {phases.map(phase => (
      ...
      {filteredPhases.map((phase) => {
        // Filter tasks by phase AND exclude Ideas
        const phaseTasks = tasks.filter(t => t.phase === phase && t.status !== TaskStatus.IDEA);
